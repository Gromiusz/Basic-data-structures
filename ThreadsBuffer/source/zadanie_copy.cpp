...
int const threadsCounts = 7;  // number of threads 13
...
public:
	std::vector<std::string> values;
	Buffer() : mutex(1), empty(0), full(bufferSize), semA(1), semB(1), semC(1), semD(1), semS(8)
	{
		available_S = 8;
		counter = 0;
	...
	std::string getA()
	{
		...
		else if (readB && readC && readD){
			// remove element from buffer
			values.erase(values.begin());
			print("A remove");
			readA = readB = readC = readD = false;
			semA.v();
			semB.v();
			semC.v();
			semD.v();
...
public:
	...
	BufferCollection(): visit_left(0), getSem(1), printQueue(1) {};
	...
	void put(std::string value)
	{
		...
		bufferCollection[selectBuffer]->put(value);
	}
...
void* threadProd(void* arg)
{
	std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> special_mess_event(0, 1);
...
int main()
{
	...
	pthread_create(&(tid[4]), NULL, threadConsD, NULL);
	...
	pthread_create(&(tid[8]), NULL, threadProd, NULL);
	// pthread_create(&(tid[9]), NULL, threadConsB, NULL);
	// pthread_create(&(tid[10]), NULL, threadProd, NULL);
	// pthread_create(&(tid[11]), NULL, threadProd, NULL);
