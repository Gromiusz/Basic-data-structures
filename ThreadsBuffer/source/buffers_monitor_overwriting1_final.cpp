...
public:
	...
	bool do_values_contain_special_mess(){
		for(auto it = values.begin()+1; it != values.end(); it++){ // begin()+1 poniewaz nie zamieniamy pierwszego miejsca
	...
	void put(std::string value){
		...
		if(is_full()) {
			if(!isSpecialMessage(value) || (isSpecialMessage(value) && !do_values_contain_special_mess()))//&& positions2.empty()))
				m.wait(full);
			//full.wait();
		}
		...
		if(special_mess.count(value)) {
			if(available_S>0){
				...
				if(!is_full()){
					...
					// zamiana istniejacej wiadomosci specjalnej na nowa. Pierwsza pozycja [0] jest omijana
					...
					// 			values[intIt] = value;
					// 			prev_intIt = intIt;
					// 			if(prev_intIt == bufferSize-1) // w przypadku dojscia do konca wektora iterujemy go od nowa
					// 				prev_intIt = 0;
					// 			break;
					// 		}
					...
					while (true) {
						...
						// Sprawdź, czy osiągnęliśmy koniec wektora
						if (it == values.end()) {
							it = values.begin();
							prev_intIt = 0;
						}
