...
private:
	...
	bool isSpecialMessage(const std::string& values) {
		return special_mess.find(values) != special_mess.end();
	}

public:
	std::vector<std::string> values;
	int buffIndex;
	Buffer(): intIt(1), prev_intIt(1) {
		available_S = 8;
	...
	bool do_values_contain_special_mess(){
		for(auto it = values.begin(); it != values.end(); it++){
			if(special_mess.find(*it) != special_mess.end()){
				return true;
			}
		}
		return false;
	...
	void put(std::string value){
		...
		for (auto it = values.begin(); it != values.end(); ++it) {
			if (special_mess.find(*it) != special_mess.end()) {
			positions2.push_back(std::distance(values.begin(), it));
			}
		}
		
		if(is_full()) {
			if(!isSpecialMessage(value) || (isSpecialMessage(value) && positions2.empty()))
				m.wait(full);
			//full.wait();
		}
		...
		if(special_mess.count(value)) {
			if(available_S>0){
				...
				if(!is_full()){
					...
					// 	intIt=1;
					// }
					// values[intIt%10] = value;
					// intIt++;
					//auto iterator = std::find_if(values.begin(), values.end(), [&](const std::string& value2) {return special_mess.find(value2) != special_mess.end();});
					//intIt = std::distance(values.begin(), iterator);

					std::vector<int> positions;
					for (auto it = values.begin(); it != values.end(); ++it) {
						if (special_mess.find(*it) != special_mess.end()) {
							positions.push_back(std::distance(values.begin(), it));
							if(positions.back() >= prev_intIt){
								intIt = positions.back();
								values[intIt] = value;
								prev_intIt = intIt;
								if(prev_intIt==10)
									prev_intIt = 1;
	...
	std::string getA()
	{
		...
		if(!special_mess.count(v)){
			...
			available_S++;
			values.erase(values.begin());
			print("A remove special");
			readA = readB = readC = readD = readE = readF = readG = readH = false;
			m.signal(conA); m.signal(conB); m.signal(conC); m.signal(conD); m.signal(conE); m.signal(conF); m.signal(conG); m.signal(conH);
...
public:
	...
	int put(std::string value)
	{
		...
		// 	tryCounter++;
		// }while(bufferCollection[selectBufferPut]->is_full() && tryCounter<10 && !isSpecialMessage(value));
		// if(tryCounter<10 || isSpecialMessage(value)){
		...
		collMon.leave();
		return result;
		/// trzeba usunac collmony z getterow i putterow
	}
	...
	std::string get(Getter getter, TimesVec& timesVec)
	{
		...
		if(bufferCollection[selectedBuffer]->is_empty()) // ==-2 all buffers have been read
		{
			result = "Err";
			collMon.leave();
			return result;
	...
	int get_random_buffer(int max, const std::vector<int>& excluded, std::string value)
	{
		...
		for (int i = 0; i <= max; ++i)
		{
			if (std::find(excluded.begin(), excluded.end(), i) == excluded.end() || (isSpecialMessage(value) && bufferCollection[i]->do_values_contain_special_mess()))
			{
				available_numbers.push_back(i);
			}
		}
